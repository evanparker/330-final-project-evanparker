...
describe("/minis", () => {
  ...
  describe("Before login", () => {
    ...
    describe("GET /", () => {
      // it("should send 401 without a token", async () => {
      //   const res = await request(server).get("/minis").send(image0);
      //   expect(res.statusCode).toEqual(401);
      // });
      // it("should send 401 with a bad token", async () => {
      //   const res = await request(server)
    ...
    describe("GET /:id", () => {
      // it("should send 401 without a token", async () => {
      //   const res = await request(server).get("/minis/123").send(image0);
      //   expect(res.statusCode).toEqual(401);
      // });
      // it("should send 401 with a bad token", async () => {
      //   const res = await request(server)
  ...
  describe("after login", () => {
    ...
    //       .post("/minis")
    //       .set("Authorization", "Bearer " + token0)
    //       .send([images[0], images[1]].map((i) => i._id));
    //     mini0Id = res0.body._id;
    //     const res1 = await request(server)
    //       .post("/minis")
    //       .set("Authorization", "Bearer " + adminToken)
    //       .send([images[0]].map((i) => i._id));
    //     mini1Id = res1.body._id;
    //   });
